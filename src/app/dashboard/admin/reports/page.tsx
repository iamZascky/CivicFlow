...
import { Loader2, FileText, Download, Printer, Filter, Calendar, BarChart3, AlertTriangle } from "lucide-react";
...
export default function AdminReportsPage() {
  ...
  const filteredComplaints = complaints.filter(c => {
    ...
    if (filterSubdistrict !== "ALL" && (c.subdistrict || "Unknown") !== filterSubdistrict) return false;
  ...
  const handleExportCSV = () => {
    ...
    // headers
    const headers = ["ID", "Title", "Description", "Category", "Priority", "Status", "Subdistrict", "Latitude", "Longitude", "Created At"];
    ...
    link.setAttribute("download", `civicflow_report_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const generateExecutiveSummary = () => {
    if (stats.total === 0) {
      return "No data matches current filters to generate executive summary.";
    }

    // Find top category
    const catCounts = filteredComplaints.reduce((acc: any, curr: any) => {
    ...
    const categoryPercentage = Math.round((maxCount / stats.total) * 100);

    return `This executive summary encapsulates Jember Regency's municipal reports compiled on ${new Date().toLocaleDateString()}. A total of ${stats.total} public complaints are included. The leading source of concern is related to '${topCategory}' issues, which comprise ${categoryPercentage}% of all filed reports. Active administrative operations indicate a public resolution rate of ${resolvedRate}%, with ${stats.resolved} complaints resolved, ${stats.inProgress} in progress, and ${stats.pending} currently pending critical evaluation. There are ${stats.critical} urgent issues classified as CRITICAL priority demanding immediate intervention.`;
  };
    ...
    <div className="w-full space-y-8 animate-fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 print:hidden">
        <div>
          ...
          <p className="text-slate-500">Filter, summarize, print, and export municipal complaint data.</p>
        </div>
      ...
      <div className="hidden print:block border-b-2 border-slate-800 pb-4 mb-6">
        <div className="flex justify-between items-end">
          <div>
            <h1 className="text-4xl font-extrabold text-slate-900 uppercase">CivicFlow Municipal Report</h1>
            <p className="text-slate-600 text-sm mt-1">Jember Regency Municipal Complaint & Sentiment Data Summary</p>
          </div>
          <div className="text-right text-xs text-slate-500 font-mono">
            Date Generated: {new Date().toLocaleDateString()}<br />
            Scope: Admin Control Center
          </div>
        </div>
      </div>
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-200/30 dark:border-slate-800/30 space-y-4 print:hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          <div className="flex flex-col">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-450 mb-1.5 ml-1">Start Date</span>
            <input
              type="date"
              value={startDate}
          ...
          <div className="flex flex-col">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-450 mb-1.5 ml-1">End Date</span>
            <input
              type="date"
              value={endDate}
          ...
          <div className="flex flex-col">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-450 mb-1.5 ml-1">Category</span>
            <CustomSelect
          ...
          <div className="flex flex-col">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-450 mb-1.5 ml-1">Priority</span>
            <CustomSelect
          ...
          <div className="flex flex-col">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-450 mb-1.5 ml-1">Status</span>
            <CustomSelect
              value={filterStatus}
          ...
          <div className="flex flex-col">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-450 mb-1.5 ml-1">Subdistrict</span>
            <CustomSelect
              value={filterSubdistrict}
              onChange={setFilterSubdistrict}
              options={[
                { value: "ALL", label: "All Subdistricts" },
                ...uniqueSubdistricts.map(sub => ({ value: sub, label: sub }))
              ]}
            />
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 print:grid-cols-4 print:gap-4 print:mb-6">
        {[
          { label: "Total Reports", value: stats.total, icon: FileText, color: "text-blue-500", bg: "bg-blue-500/10" },
          { label: "Resolution Rate", value: `${resolvedRate}%`, icon: BarChart3, color: "text-emerald-500", bg: "bg-emerald-500/10" },
          { label: "Critical Priority", value: stats.critical, icon: AlertTriangle, color: "text-red-500", bg: "bg-red-500/10" },
          { label: "Pending Review", value: stats.pending, icon: Loader2, color: "text-amber-500", bg: "bg-amber-500/10" },
        ].map((stat, i) => (
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-200/30 dark:border-slate-800/30 space-y-3 print:border-slate-300 print:bg-white print:p-4 print:shadow-none print:mb-6">
        <h3 className="text-base font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2 print:text-black">
          <BarChart3 className="size-5 text-blue-500 print:hidden" />
          Executive Overview Summary
        </h3>
        <p className="text-sm text-slate-600 dark:text-slate-350 leading-relaxed text-justify print:text-slate-800">
      ...
      <div className="glass-panel rounded-2xl border border-slate-200/30 dark:border-slate-800/30 overflow-hidden print:border-slate-300 print:bg-white print:shadow-none">
        <div className="p-5 border-b border-slate-200/40 dark:border-slate-800/50 flex justify-between items-center print:hidden">
          <h3 className="font-bold text-base">Report Preview Table</h3>
          <span className="text-xs font-semibold text-slate-500 bg-slate-100/80 dark:bg-slate-800/50 px-2.5 py-1 rounded-full border border-slate-200/50 dark:border-slate-700/50">
            {filteredComplaints.length} Records Match
          </span>
        </div>

        <div className="overflow-x-auto">
          {filteredComplaints.length === 0 ? (
            <div className="p-10 text-center text-slate-500">No complaints match your query parameters.</div>
          ) : (
