...
import { Loader2, History, RotateCcw, Trash2, Archive, CheckSquare, Square } from "lucide-react";
...
export default function AdminHistoryPage() {
  ...
  const fetchComplaints = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Filter only resolved or rejected complaints, and only master complaints
        setComplaints(data.filter((c: any) => 
          !c.parentId && (c.status === "RESOLVED" || c.status === "REJECTED")
        ));
      }
    } catch (e) {
      console.error(e);
  ...
  const handleBulkArchive = async () => {
    ...
    try {
      ...
      console.error(e);
      alert("Failed to archive complaints");
    }
  };
    ...
    <div className="w-full flex-1 flex flex-col space-y-6 animate-fade-in-up">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 shrink-0">
        <div>
          ...
          <p className="text-slate-500 mt-1">Archived reports that have been resolved or rejected.</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            ...
            className={`flex items-center justify-center gap-2 px-4 py-2 rounded-xl font-semibold transition-all border ${
              selectedIds.size > 0 
                ? 'bg-emerald-50 hover:bg-emerald-100 text-emerald-600 border-emerald-200 dark:bg-emerald-900/20 dark:hover:bg-emerald-900/40 dark:text-emerald-400 dark:border-emerald-800/50 cursor-pointer' 
      ...
      <div className="flex items-center gap-2 px-2 pb-2 border-b border-slate-200 dark:border-slate-800">
        <button onClick={handleSelectAll} className="flex items-center gap-2 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 transition-colors">
          {allSelected ? <CheckSquare className="size-5 text-blue-500" /> : <Square className="size-5" />}
            ...
            return (
              <div
                ...
                className={`glass-panel p-4 md:p-6 rounded-2xl flex flex-col md:flex-row gap-4 border transition-all ${
                  isSelected 
                    ? 'border-blue-400 shadow-md bg-blue-50/30 dark:bg-blue-900/10 dark:border-blue-500/50' 
                    : 'border-slate-200/50 dark:border-slate-800/50 hover:shadow-md'
                }`}
              >
                <div className="flex items-start gap-4">
                  <button onClick={() => handleSelect(c.id)} className="mt-1 shrink-0 text-slate-400 hover:text-blue-500 transition-colors">
                    {isSelected ? <CheckSquare className="size-5 text-blue-500" /> : <Square className="size-5" />}
                  </button>
                </div>
                ...
                <div className="flex-1 flex flex-col">
                  ...
                  {c.status === 'REJECTED' && c.rejectionReason && (
                    <div className="mb-3 p-3 bg-slate-100 dark:bg-slate-800/80 rounded-xl border border-slate-300 dark:border-slate-700">
                      <p className="text-sm text-slate-700 dark:text-slate-300">
                        <span className="font-semibold mr-1">Rejection Reason:</span>
                        {c.rejectionReason}
                      </p>
                    </div>
                  )}
